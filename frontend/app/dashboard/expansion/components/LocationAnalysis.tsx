...
export default function LocationAnalysis() {
  ...
  const fetchTopLocations = useCallback(async () => {
    try {
      const res = await fetch("http://127.0.0.1:8000/get_missing_locations");
  ...
  const fetchCityAnalysis = useCallback(async (cities: string[]) => {
    ...
    await Promise.all(
      cities.map(async (city) => {
        try {
          const res = await fetch("http://127.0.0.1:8000/analyze-location", {
            method: "POST",
  ...
  const sendWhatsApp = async (city: string) => {
    try {
      const res = await fetch(`http://127.0.0.1:8000/send_whatsapp?city=${city}`, {
        method: "POST",
      });
